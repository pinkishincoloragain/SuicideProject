...
def analyze(in_line,cnt):
    ...
    with open("result/"+"result_" +'suicide_detected.csv', mode='a+', encoding='utf-8') as outfile_1:
        writing_counter = 0
        ...
        if len(appeared) > 0:
            ...
            for app in range(case):
                with open("result/power_word_" + appeared[app] + '_.csv', mode='a+',
                          encoding='utf-8') as outfile_2:
                    outfile_2.write(f"{filename}_{cnt} case: {case}\nPower words: {appeared}\noriginal: {original}\n")
                with open("result/power_word_count_" + str(len(appeared)) + '_.csv', mode='a+',
                          encoding='utf-8') as outfile_3:
                    outfile_3.write(f"{filename}_{cnt} case: {case}\nPower words: {appeared}\noriginal: {original}\n")
